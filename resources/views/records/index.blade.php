        ...
        <section class="search-records">
            <form method="GET" action="{{action('FormSearchController@keywordSearch',['pid' => $form->project_id, 'fid' => $form->id])}}" class="keyword-search-js">
                <div class="form-group search-input mt-xl">
                    ...
                    {!! Form::text('keywords', null, ['class' => 'text-input keywords-get-js', 'placeholder' => 'Type space separated keywords (Use double quotes to create key phrases)']) !!}
                </div>
                <div class="form-group search-input mt-xl">
                    {!! Form::label('method','or / and / kid') !!}
                    {!! Form::select('method',[0 => 'OR',1 => 'AND',2 => 'KID'], 0, ['class' => 'single-select']) !!}
                </div>
