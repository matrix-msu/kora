...
<div class="form-group inline-form-group mt-xl">
    <div class="form-group">
        <label>{{$title}}Default Hour</label>
        <select name="default_hour{{$seq}}" id='default_hour{{$seq}}' class="single-select {{$defClass}}" data-placeholder="Select an Hour">
            @php
                for($i=0;$i<24;$i++) {
                    if(!is_null($default) && $default['hour'] == $i)
    ...
    <div class="form-group">
        <label>{{$title}}Default Minute</label>
        <select name="default_minute{{$seq}}" id='default_minute{{$seq}}' class="single-select {{$defClass}}" data-placeholder="Select a Minute">
            @php
                for($i=0;$i<60;$i++) {
                    if(!is_null($default) && $default['minute'] == $i)
    ...
    <div class="form-group">
        <label>{{$title}}Default Second</label>
        <select name="default_second{{$seq}}" id='default_second{{$seq}}' class="single-select {{$defClass}}" data-placeholder="Select a Second">
            @php
                for($i=0;$i<60;$i++) {
                    if(!is_null($default) && $default['second'] == $i)
