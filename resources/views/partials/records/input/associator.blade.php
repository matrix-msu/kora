@php
    ...
    } else if($editRecord && !is_null($record->{$flid})) {
        $selected = array();
...
<div class="form-group associator-input">
    <div class="form-group mb-xl">
        ...
        <input type="text" class="text-input assoc-search-records-js" placeholder="Enter search term or KID to find associated records (populated below)"
            search-url="{{ action('AssociatorSearchController@assocSearch',['pid' => $form->project_id,'fid'=>$form->id, 'flid'=>$flid]) }}" @if(isset($seq))combo="{{$seq}}"@endif>
    ...
    <div class="form-group mt-xs mb-xl">
        <label>@if(!isset($seq) && $field['required'])<span class="oval-icon"></span> @endif Selected Associations</label>
        <span class="error-message"></span>
        {!! Form::select($fieldLabel, $value, $selected, ['class' => 'multi-select assoc-default-records-js preset-clear-chosen-js',
            'multiple', "data-placeholder" => "Selected Associated Records will appear here.", 'id' => $fieldDivID]) !!}
