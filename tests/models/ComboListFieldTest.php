...
TEXT;
    ...
    public function test_rollback() {
        ...
        $combo = new ComboListField();
        $combo->flid = $field->flid;
        $combo->rid = $record->rid;
        $combo->fid = $field->fid;
        $combo->save();

        $combo->addData($old, Field::_NUMBER, Field::_GENERATED_LIST);

        $revision = RevisionController::storeRevision($record->rid, Revision::CREATE);

        $new = [
            "[!f1!]4[!f1!][!f2!]Jangus[!]Crust[!f2!]",
            "[!f1!]5[!f1!][!f2!]Microsoft[!]Cisco[!f2!]",
