...
class SavePluginUsersTable extends Command implements ShouldQueue {

    /*
    |--------------------------------------------------------------------------
    | Save Plugin Users Table
    |--------------------------------------------------------------------------
    |
    | This command handles the backup of the plugin users table
    |
    */
    ...
    public function handle() {
        ...
        DB::table('plugin_users')->orderBy('plugin_id')->chunk(500, function($pluginUsers) use ($table_path, $row_id) {
            $count = 0;
